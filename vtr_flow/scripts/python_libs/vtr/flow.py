...
import fault_tolerant_routing_mux as ftrm
import vtr
...
def run(
    ...
    if should_run_stage(VtrStage.VPR, start_stage, end_stage):
        ...
        if route_fixed_w:
            ...
            if "write_rr_graph" in vpr_args or "analysis" in vpr_args or "route" in vpr_args or "fault_sim" in vpr_args:
                do_second_run = True

            if "fault_sim" in vpr_args:
                # Set sim run and pop argument for VPR
                do_fault_sim = True
                vpr_args.pop("fault_sim")

                # Default sim args
                equal_p = None
                pSA0 = pSA1 = pUD = 0.03
                cell_type = ftrm.MemCell

                if "equal_fault" in vpr_args:
                    equal_p = float(vpr_args["equal_fault"])
                    vpr_args.pop("equal_fault")
                else:
                    if "pSA0" in vpr_args:
                        pSA0 = float(vpr_args["pSA0"])
                        vpr_args.pop("pSA0")
                    if "pSA1" in vpr_args:
                        pSA1 = float(vpr_args["pSA1"])
                        vpr_args.pop("pSA1")
                    if "pUD" in vpr_args:
                        pUD = float(vpr_args["pUD"])
                        vpr_args.pop("pUD")

                if "cell_type" in vpr_args:
                    cell_type = getattr(ftrm, vpr_args["cell_type"])
                    vpr_args.pop("cell_type")

                # Save rr_graph for simulation
                if "write_rr_graph" in vpr_args:
                    fault_sim_rr_graph = temp_dir / vpr_args["write_rr_graph"]
                else:
            ...
            if do_fault_sim:
                ...
                if equal_p is not None:
                    fault_sim = ftrm.FaultSimulator(
                        cell_type=cell_type,
                        rr_graph_file=fault_sim_rr_graph,
                        p=equal_p
                    )
                else:
                    fault_sim = ftrm.FaultSimulator(
                ...
                vpr_args["read_rr_graph"] = Path(fault_sim.get_faulty_rr_graph()).name
